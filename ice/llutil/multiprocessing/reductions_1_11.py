...
class SharedCache(dict):
    ...
    def __init__(self):
        ...
        # possible deadlocks, following python multiprocess library design.
        self._after_fork()
...
def rebuild_storage_filename(cls, manager, handle, size, dtype=None):
    storage: Union[torch._TypedStorage, torch._UntypedStorage] = storage_from_cache(
        cls, handle
    )
    if storage is not None:
    ...
    if dtype is None:
        ...
        untyped_storage: torch._UntypedStorage = (
            torch._UntypedStorage._new_shared_filename(manager, handle, byte_size)
        )
        storage = torch._TypedStorage(wrap_storage=untyped_storage, dtype=dtype)
    shared_cache[handle] = StorageWeakRef(storage)
    return storage._shared_decref()
...
def reduce_storage(storage):
    ...
    if storage.is_cuda:
        ...
        df = multiprocess.reduction.DupFd(fd)
        cache_key = fd_id(fd)
        metadata = (df, size)
